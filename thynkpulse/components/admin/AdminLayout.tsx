'use client'
// components/admin/AdminLayout.tsx  (ThynkPulse)
// Updated: added Notifications, SEO Manager, AI Chatbot nav items
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { useState } from 'react'
import {
  LayoutDashboard, FileText, Pencil, Users, FileCheck, Palette, LogOut,
  Menu, X, GraduationCap, Settings, MessageSquare, Plug, BarChart2,
  Bell, Search, Bot,
} from 'lucide-react'
...
const NAV = [
  ...
  { icon: Bot,             label: 'AI Chatbot',      href: '/admin/chatbot'        },
...
export function AdminLayout({ children, title, subtitle }: {
  ...
  const Sidebar = ({ onClose }: { onClose?: () => void }) => (
    <aside style={{ width: 240, background: 'var(--admin-sidebar-bg, #1A1208)', display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
      {/* Logo */}
      <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>
          ...
          <span style={{ fontFamily: 'var(--font-serif)', fontSize: '16px', fontWeight: 900, color: '#fff' }}>
            Thynk <em style={{ fontStyle: 'normal', color: 'var(--teal3)' }}>Pulse</em>
          </span>
        </Link>
        {onClose && (
      ...
      <div style={{ padding: '14px', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ fontSize: '13px', fontWeight: 600, color: '#fff', fontFamily: 'var(--font-sans)' }}>
              {user?.profile?.fullName || 'Admin'}
            </div>
            <div style={{ fontSize: '10px', background: 'rgba(10,95,85,.2)', color: 'var(--teal3)', padding: '2px 8px', borderRadius: '100px', display: 'inline-block', marginTop: '2px', fontFamily: 'var(--font-mono)', fontWeight: 700, letterSpacing: '.04em' }}>
              ADMIN
            </div>
          </div>
        </div>
          ...
          return (
            <Link
              ...
              style={{
                ...
                background: active ? 'var(--admin-sidebar-active, var(--teal))' : 'transparent',
                color: active ? 'var(--admin-sidebar-active-color, #fff)' : 'rgba(255,255,255,.5)',
              }}>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--admin-bg, var(--cream2))', overflow: 'hidden', fontFamily: 'var(--font-sans)' }}>
