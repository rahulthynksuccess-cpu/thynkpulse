...
import { LayoutDashboard, FileText, Pencil, Users, FileCheck, Palette, LogOut, Menu, X, GraduationCap, Settings, MessageSquare, Plug, BarChart2 } from 'lucide-react'
    ...
    <aside style={{ width:240, background:'var(--admin-sidebar-bg, #1A1208)', display:'flex', flexDirection:'column', height:'100%', flexShrink:0 }}>
      ...
      <nav style={{ flex:1, padding:'10px', display:'flex', flexDirection:'column', gap:'2px', overflowY:'auto' }}>
        {NAV.map(({ icon: Icon, label, href }) => {
          ...
          return (
            <Link key={href} href={href} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'10px', fontFamily:'var(--font-sans)', fontSize:'13px', fontWeight:500, textDecoration:'none', transition:'all .18s', background: active ? 'var(--admin-sidebar-active, var(--teal))' : 'transparent', color: active ? 'var(--admin-sidebar-active-color, #fff)' : 'rgba(255,255,255,.5)' }}>
