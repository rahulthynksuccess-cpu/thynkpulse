...
const TRENDING_DEFAULT = [
  ...
  { num:'04', tag:'Educator · Story',  title:'I Quit a Private School to Teach in a Government School. Here is Why.', reads:'19.8K', likes:'1.2K', comments:'317' },
  { num:'05', tag:'EdTech · Funding',  title:'What EdTech Investors Actually Look for in 2025',                  reads:'17.3K', likes:'428', comments:'71'  },
]
const TOPICS_DEFAULT = ['AI in Education','School Leadership','EdTech Sales','NEP 2020','Teacher Training','Ed-Finance','Curriculum Design','Student Wellbeing','Higher Education','Vocational Ed','STEM','Ed Policy']
...
export function TrendingSection() {
  ...
  const tc = tcRaw ?? {}
  const rawTrending = Array.isArray(tc?.items) ? tc.items : []
  const TRENDING = rawTrending.some((t:any) => t.title)
    ? rawTrending.map((t:any, i:number) => ({
        ...TRENDING_DEFAULT[i],
        ...t,
        tag:   t.tag   || TRENDING_DEFAULT[i]?.tag,
        title: t.title || TRENDING_DEFAULT[i]?.title,
      }))
    : TRENDING_DEFAULT
