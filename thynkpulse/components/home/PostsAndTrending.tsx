    ...
    <motion.div initial={{ opacity:0, y:22 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:(index%4)*.08 }}>
      <Link href={`/post/${post.slug}`} className="pcard" style={{ height:'100%' }}>
        ...
        <div style={{ padding:'22px', flex:1, display:'flex', flexDirection:'column' }}>
          <div style={{ fontFamily:'var(--font-serif)', fontSize: big ? 26 : 18, fontWeight:600, color:'var(--ink)', lineHeight:1.35, marginBottom:'8px' }}>
            {post.title}
          </div>
          {post.excerpt && (
...
export function PostsSection() {
  ...
  return (
    <section id="posts" style={{ padding:'96px 5%', background:'var(--cream)' }}>
      <div className="eyebrow"><div className="eyebrow-line" /><span className="eyebrow-text">Latest from the community</span></div>
      <h2 className="s-title">Fresh Ideas,<br /><em>Real Voices</em></h2>
      <p className="s-desc">Articles from educators, EdTech founders, sales pros, and innovators shaping education worldwide.</p>
...
const TOP_WRITERS = [
  { initials:'RK', name:'Rajesh Kumar', role:'EdTech Founder',   av:'av-teal'  },
  { initials:'PS', name:'Priya Sharma', role:'Teacher, Delhi',   av:'av-coral' },
  { initials:'AM', name:'Arjun Mehta',  role:'Sales Director',   av:'av-gold'  },
  { initials:'NV', name:'Nalini Verma', role:'Researcher, IIT',  av:'av-plum'  },
]
...
export function TrendingSection() {
  ...
  return (
    <section id="trending" ref={ref} style={{ padding:'96px 5%', background:'#fff' }}>
      <div className="eyebrow"><div className="eyebrow-line" /><span className="eyebrow-text">What the community is reading</span></div>
      <h2 className="s-title">Trending<br /><em>This Week</em></h2>

      <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:'40px', marginTop:'56px', alignItems:'start' }}>
        ...
        <div>
          {TRENDING.map((t, i) => (
            <motion.a key={t.num} href="#" initial={{ opacity:0, y:18 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ delay:i*.09 }}
              ...
              <div style={{ fontFamily:'var(--font-serif)', fontSize:'32px', fontWeight:900, color:'var(--parchment)', lineHeight:1, flexShrink:0, minWidth:'44px' }}>{t.num}</div>
              <div>
                <div style={{ fontFamily:'var(--font-mono)', fontSize:'11px', fontWeight:700, letterSpacing:'1px', textTransform:'uppercase', color:'var(--coral)', marginBottom:'5px' }}>{t.tag}</div>
                <div style={{ fontFamily:'var(--font-serif)', fontSize:'17px', fontWeight:600, color:'var(--ink)', lineHeight:1.35, marginBottom:'8px', transition:'color .2s' }}>{t.title}</div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
          ...
          <div className="sidebar-card">
            ...
            {TOP_WRITERS.map(w => (
              <div key={w.name} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 0', borderBottom:'1px solid var(--border2)' }}>
                <div className={`avatar ${w.av}`} style={{ width:36, height:36, fontSize:'14px', borderRadius:'10px' }}>{w.initials}</div>
                <div>
                  <div style={{ fontSize:'13px', fontWeight:600, color:'var(--ink)' }}>{w.name}</div>
                  <div style={{ fontSize:'11px', color:'var(--muted)' }}>{w.role}</div>
                </div>
                <button onClick={() => setFollowed(p => ({ ...p, [w.name]: !p[w.name] }))}
                  style={{ marginLeft:'auto', fontSize:'12px', fontWeight:600, color: followed[w.name] ? '#fff' : 'var(--teal)', background: followed[w.name] ? 'var(--teal)' : 'transparent', padding:'5px 12px', borderRadius:'50px', border:'1px solid rgba(10,95,85,.2)', cursor:'pointer', fontFamily:'var(--font-sans)', transition:'all .2s', flexShrink:0 }}>
                  {followed[w.name] ? 'Following ✓' : 'Follow'}
                </button>
              </div>
            ))}
          </div>
