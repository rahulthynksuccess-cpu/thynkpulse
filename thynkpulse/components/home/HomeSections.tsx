    ...
    <section id="community" style={{ padding:'96px 5%', background:'var(--community-bg, #fff)' }}>
      ...
      <div className="comm-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px', marginTop:'56px' }}>
        {COMMUNITIES.map((comm, i) => (
          <motion.div key={comm.title} initial={{ opacity:0, y:22 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:i*.08 }}
            ...
            <div style={{ fontFamily:'var(--font-serif)', fontSize:'var(--community-card-title-size, 20px)', fontWeight:700, color:'var(--ink)', marginBottom:'10px' }}>{comm.title}</div>
            <div style={{ fontSize:'var(--community-card-desc-size, 13px)', color:'var(--muted)', lineHeight:1.7, marginBottom:'18px' }}>{comm.desc}</div>
        ...
        <div style={{ position:'relative' }}>
          ...
          <div className="card" style={{ padding:'30px', background:'var(--writers-card-bg-home,#fff)' }}>
            <div style={{ display:'flex', alignItems:'flex-start', gap:'14px', marginBottom:'22px' }}>
              ...
              <div>
                <div style={{ fontFamily:'var(--font-serif)', fontSize:'var(--writers-name-size,20px)', fontWeight:700, color:'var(--writers-name-color,var(--ink))' }}>Ananya Krishnan</div>
                <div style={{ fontSize:'var(--writers-role-size,12px)', color:'var(--writers-role-color,var(--muted))', marginTop:'2px' }}>EdTech Product Lead · Mumbai</div>
              </div>
    ...
    <section id="join" style={{ padding:'96px 5%', background:'var(--cta-section-bg,var(--cream))' }}>
      <div style={{ background:'var(--teal)', borderRadius:'32px', padding:'80px 72px', position:'relative', overflow:'hidden', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:'10px', maxWidth:'460px', margin:'0 auto 20px', position:'relative', zIndex:2 }}>
          ...
          <Link href="/register" style={{ flexShrink:0, padding:'14px 28px', borderRadius:'var(--radius)', background:'var(--cta-btn-bg,var(--coral))', color:'var(--cta-btn-color,#fff)', fontSize:'var(--cta-btn-size,15px)', fontWeight:600, fontFamily:'var(--font-sans)', textDecoration:'none', display:'inline-flex', alignItems:'center' }}>Join Free →</Link>
        </div>
