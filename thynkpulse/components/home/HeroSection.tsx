...
export function HeroSection() {
  ...
  return (
    <section style={{ background:'var(--cream)', minHeight:'90vh', display:'flex', alignItems:'center', paddingTop:'80px', paddingBottom:'60px', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:'1160px', margin:'0 auto', padding:'0 5%', width:'100%' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 420px', gap:'80px', alignItems:'center' }}>
          ...
          <div>
            ...
            <motion.h1 initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.7,delay:.18}} style={{
            ...
            <motion.p initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.7,delay:.26}} style={{ fontFamily:'var(--font-sans)', fontSize:'17px', fontWeight:300, color:'var(--muted)', lineHeight:1.75, maxWidth:'480px', marginBottom:'40px' }}>
              {c.subtitle}
            </motion.p>

            <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:.6,delay:.32}} style={{ display:'flex', gap:'12px', flexWrap:'wrap', marginBottom:'32px' }}>
            ...
            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.4}} style={{ display:'flex', alignItems:'center', gap:'12px', fontSize:'13px', color:'var(--muted)', fontFamily:'var(--font-sans)' }}>
          ...
          {/* Card */}
          <motion.div initial={{opacity:0,x:30}} animate={{opacity:1,x:0}} transition={{duration:.8,delay:.3}} style={{ background:'#fff', borderRadius:'24px', border:'1px solid var(--border)', overflow:'hidden', boxShadow:'0 24px 80px rgba(26,18,8,.12)' }}>
      ...
      <style>{`
        @media(max-width:1024px){
          .hero-h1-override{font-size:64px !important;}
        }
        @media(max-width:768px){
          section[style*="minHeight:'90vh'"]{padding-top:90px !important;}
          div[style*="gridTemplateColumns:'1fr 420px'"]{grid-template-columns:1fr !important; gap:0 !important;}
          div[style*="gridTemplateColumns: '1fr 420px'"]{grid-template-columns:1fr !important; gap:0 !important;}
        }
      `}</style>
    </section>
