...
const inp: React.CSSProperties = { fontFamily: 'var(--font-sans)', width: '100%', padding: '12px 16px', border: '1.5px solid var(--parchment)', borderRadius: 'var(--radius)', fontSize: '14px', color: 'var(--ink)', outline: 'none', background: '#fff', boxSizing: 'border-box', transition: 'border-color .2s' }
...
export default function RegisterPage() {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '24px' }}>
        {[1, 2].map(s => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {s < 2 && <div style={{ width: 40, height: 2, borderRadius: 2, background: step > s ? 'var(--teal)' : 'var(--parchment)', transition: 'background .25s' }} />}
          </div>
        ))}
      </div>
      ...
      {step === 1 ? (
        <>
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '26px', fontWeight: 900, color: 'var(--ink)', marginBottom: '4px', letterSpacing: '-0.5px' }}>Join Thynk Pulse</h1>
          <p style={{ fontSize: '14px', color: 'var(--muted)', marginBottom: '22px', fontWeight: 300 }}>India's platform for everyone in education</p>
        ...
        <form onSubmit={handleRegister}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', padding: '10px 14px', background: 'rgba(10,95,85,.05)', borderRadius: '10px', border: '1.5px solid rgba(10,95,85,.12)' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <button type="button" onClick={() => setStep(1)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', color: 'var(--muted)', padding: 0, textDecoration: 'underline', fontFamily: 'var(--font-sans)' }}>Change</button>
            </div>
          </div>
          <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '24px', fontWeight: 900, color: 'var(--ink)', marginBottom: '20px', letterSpacing: '-0.3px' }}>Your account details</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '22px' }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input style={{ ...inp, paddingRight: '44px' }} type={showPw ? 'text' : 'password'} placeholder="Min 8 characters" value={form.password} onChange={e => set('password', e.target.value)} required />
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button type="button" onClick={() => setStep(1)}
              style={{ padding: '12px 18px', borderRadius: '12px', border: '1.5px solid var(--parchment)', background: '#fff', fontFamily: 'var(--font-sans)', fontSize: '13px', color: 'var(--muted)', cursor: 'pointer' }}>← Back</button>
