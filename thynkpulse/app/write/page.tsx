...
export default function WritePage() {
  ...
  // Block writing if profile not complete
  if (isAuthenticated && !user?.profile?.profileCompleted) {
    return (
    ...
    <div style={{ minHeight:'100vh', background:'var(--cream)' }}>
      ...
      <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(253,246,236,.95)', backdropFilter:'blur(20px)', borderBottom:'1px solid var(--border)', padding:'0 5%', height:'60px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
          ...
          <button onClick={() => { if (!form.title || !form.content) { toast.error('Title and content are required'); return } mutation.mutate('pending') }}
        ...
        {!preview ? (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            ...
            <textarea
              ...
              rows={2}
              style={{ width:'100%', fontFamily:'var(--font-serif)', fontSize:'clamp(28px,4vw,44px)', fontWeight:900, color:'var(--ink)', letterSpacing:'-1px', lineHeight:1.15, border:'none', outline:'none', background:'transparent', resize:'none', marginBottom:'12px', placeholder:'color:var(--parchment)' }}
            />
            ...
            {/* Content editor */}
            <textarea
              placeholder={`Start writing your post...\n\nShare your experiences, insights, and ideas with India's education community.\n\nTips:\n• Be specific and personal — real stories resonate\n• Use clear headings to structure long posts\n• End with a call to discussion`}
              value={form.content}
              onChange={e => set('content', e.target.value)}
              style={{ width:'100%', minHeight:'480px', fontFamily:'var(--font-sans)', fontSize:'17px', color:'var(--ink)', lineHeight:1.8, border:'none', outline:'none', background:'transparent', resize:'none', fontWeight:300 }}
            />
          </motion.div>
        ) : (
