...
export default function AdminUsersPage() {
  ...
  const {data,isLoading,isError,error} = useQuery<{data:any[];total:number}>({
      ...
      <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:'14px',overflow:'hidden'}}>
        ...
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {isError
                ? <tr><td colSpan={13} style={{padding:'32px',textAlign:'center',color:'var(--coral)',fontFamily:'var(--font-sans)',fontSize:'13px'}}>
                    ⚠ Error: {(error as Error)?.message}<br/>
                    <span style={{fontSize:'11px',color:'var(--muted)'}}>Try logging out and back in.</span>
                  </td></tr>
                : isLoading
