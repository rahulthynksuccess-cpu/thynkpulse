  ...
  return (
    <div style={fieldWrap}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
        <input type="range" min={min} max={max} value={Number(value)||16}
          onChange={e => onChange(Number(e.target.value))}
          style={{ flex:1, accentColor:'var(--teal)' }} />
        <span style={{ fontFamily:'var(--font-mono)', fontSize:'12px', fontWeight:700, color:'var(--teal)', minWidth:'48px', textAlign:'right' as const }}>
          {value || min}{unit}
        </span>
      </div>
    </div>
...
function ColorField({ label, value, onChange }: any) {
  return (
    <div style={fieldWrap}>
      <label style={lbl}>{label}</label>
      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
        <input type="color" value={value || '#1A1208'}
          onChange={e => onChange(e.target.value)}
          style={{ width:'40px', height:'36px', border:'1.5px solid var(--parchment)', borderRadius:'8px', padding:'2px', cursor:'pointer', background:'none' }} />
        <input type="text" value={value || ''}
          onChange={e => onChange(e.target.value)}
          placeholder="#1A1208"
          style={{ ...{width:'100%', padding:'9px 12px', background:'#fff', border:'1.5px solid var(--parchment)', borderRadius:'9px', fontSize:'13px', fontFamily:'var(--font-mono)', color:'var(--ink)', outline:'none', boxSizing:'border-box' as const} }} />
      </div>
    </div>
  )
}
  ...
  return (
    <div style={fieldWrap}>
      ...
      <select value={value || ''} onChange={e => onChange(e.target.value)}
        style={{ width:'100%', padding:'9px 12px', background:'#fff', border:'1.5px solid var(--parchment)', borderRadius:'9px', fontSize:'13px', fontFamily:'var(--font-sans)', color:'var(--ink)', outline:'none', cursor:'pointer' }}>
...
function HeroEditor({ val, onChange }: any) {
  ...
  return (
    <>
      {/* ── Text Content ── */}
      <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase' as const, color:'var(--muted)', marginBottom:'12px', paddingBottom:'6px', borderBottom:'1px solid var(--border)' }}>📝 Text Content</div>
      ...
      {/* ── Typography ── */}
      <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase' as const, color:'var(--muted)', margin:'20px 0 12px', paddingBottom:'6px', borderBottom:'1px solid var(--border)' }}>🔤 H1 Typography</div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
        <SelectField label="H1 Font Family" value={val.h1FontFamily || 'Fraunces'} onChange={(v:string)=>set('h1FontFamily',v)} options={FONT_OPTIONS} />
        <ColorField  label="H1 Text Colour" value={val.h1Color || '#1A1208'} onChange={(v:string)=>set('h1Color',v)} />
      </div>
      <NumberField label="H1 Font Size (desktop)" value={val.h1FontSize || 84} min={32} max={160} onChange={(v:number)=>set('h1FontSize',v)} />

      <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase' as const, color:'var(--muted)', margin:'16px 0 12px', paddingBottom:'6px', borderBottom:'1px solid var(--border)' }}>📝 Subtitle Typography</div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
        <SelectField label="Subtitle Font"   value={val.subFontFamily || 'Outfit'} onChange={(v:string)=>set('subFontFamily',v)} options={FONT_OPTIONS} />
        <ColorField  label="Subtitle Colour" value={val.subColor || '#7A6A52'} onChange={(v:string)=>set('subColor',v)} />
      </div>
      <NumberField label="Subtitle Font Size" value={val.subFontSize || 17} min={12} max={28} onChange={(v:number)=>set('subFontSize',v)} />

      <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase' as const, color:'var(--muted)', margin:'16px 0 12px', paddingBottom:'6px', borderBottom:'1px solid var(--border)' }}>🎨 Background</div>
      <ColorField label="Hero Section Background" value={val.heroBg || '#FDF6EC'} onChange={(v:string)=>set('heroBg',v)} />
    </>
  )
}
