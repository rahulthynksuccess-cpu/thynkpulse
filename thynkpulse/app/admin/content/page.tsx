...
export default function AdminContentPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success('✅ Saved! Changes live on site.')
      setUnsaved(false)
    },
  ...
  return (
    <AdminLayout title="Content & Style Manager" subtitle="Edit any text, colour, size or button — live preview updates instantly">
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px', padding:'11px 16px', background:'#fff', border:'1px solid var(--border)', borderRadius:'12px' }}>
        <div style={{ width:8, height:8, borderRadius:'50%', background: unsaved?'var(--gold)':'#4ADE80', boxShadow: unsaved?'0 0 8px rgba(201,146,42,.5)':'0 0 8px rgba(74,222,128,.4)', flexShrink:0 }} />
        <span style={{ fontSize:'12px', color:'var(--muted)', flex:1, fontFamily:'var(--font-sans)' }}>
          {unsaved ? '⚠ Unsaved — click Save to apply to live site' : '✓ All changes saved and live'}
        </span>
        <button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || !unsaved}
          style={{ display:'flex', alignItems:'center', gap:'7px', padding:'9px 22px', borderRadius:'9px', background:'var(--teal)', border:'none', color:'#fff', cursor:(saveMutation.isPending||!unsaved)?'not-allowed':'pointer', fontSize:'13px', fontWeight:700, fontFamily:'var(--font-sans)', opacity:(saveMutation.isPending||!unsaved)?.6:1, boxShadow:'0 4px 14px rgba(10,95,85,.3)' }}>
