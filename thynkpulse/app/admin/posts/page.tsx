...
export default function AdminPostsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fresh token read every time - avoids stale auth issue
      const token = typeof window !== 'undefined' ? (localStorage.getItem('tp_token') || document.cookie.match(/tp_token=([^;]+)/)?.[1] || '') : ''
      if (!token) throw new Error('Not authenticated')
      return apiGet(`/admin/posts?status=${tab}&search=${search}&page=${page}&limit=20`)
    },
    staleTime: 60 * 1000,
  })
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '20px' }}>
        {TABS.map((t, i) => (
          <motion.div key={t} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay: i*.06 }}
            ...
            style={{ background: '#fff', border: `2px solid ${tab===t ? STATUS_STYLE[t].color : 'var(--border)'}`, borderRadius: '12px', padding: '16px', cursor: 'pointer', transition: 'all .15s', background: tab===t ? `${STATUS_STYLE[t].bg}` : '#fff' }}>
                            ...
                            <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                              ...
                              {p.status !== 'rejected' && (
                                <button onClick={() => {
                                  ...
                                  updateMutation.mutate({ id: p.id, status: 'rejected', rejectionReason: reason || '' })
                                }}
