...
export async function GET(req: NextRequest) {
  ...
  if (search) {
    conditions.push(`(COALESCE(up.full_name,'') ILIKE $${idx} OR COALESCE(u.email,'') ILIKE $${idx} OR COALESCE(u.phone,'') ILIKE $${idx} OR COALESCE(up.company_name,'') ILIKE $${idx} OR COALESCE(up.institute_name,'') ILIKE $${idx})`)
    params.push(`%${search}%`)
    idx++
  }
  ...
  try {
    ...
    return Response.json({
      data: rows.rows.map(r => ({
        ...
        totalExp: r.total_exp, linkedinUrl: r.linkedin_url,
        location: r.location, interests: r.interests,
        postCount: r.post_count, profileCompleted: r.profile_completed,
