...
export async function POST(req: NextRequest) {
  // Check profile complete before allowing comments
  const _token = getTokenFromHeader(req.headers.get('authorization') || '')
  if (_token) {
    const _payload = verifyToken(_token)
    if (_payload) {
      const profileCheck = await db.query('SELECT profile_completed FROM user_profiles WHERE user_id=$1', [_payload.userId])
      if (!profileCheck.rows[0]?.profile_completed) {
        return Response.json({ error: 'Complete your profile before commenting.', code: 'PROFILE_INCOMPLETE' }, { status: 403 })
      }
    }
  }
