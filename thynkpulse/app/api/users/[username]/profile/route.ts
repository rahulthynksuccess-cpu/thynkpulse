...
export async function GET(req: NextRequest, { params }: { params: { username: string } }) {
  try {
    ...
    return Response.json({
      id: r.id, email: r.email, phone: r.phone, role: r.role, isVerified: r.is_verified,
      profile: {
        fullName: r.full_name, gender: r.gender, designation: r.designation,
        instituteName: r.institute_name, companyName: r.company_name,
        contactNumber: r.contact_number, emailId: r.email_id, totalExp: r.total_exp,
        introduction: r.introduction, avatarUrl: r.avatar_url,
        linkedinUrl: r.linkedin_url, websiteUrl: r.website_url,
        postCount: r.post_count, followerCount: r.follower_count,
        followingCount: r.following_count, totalReads: r.total_reads,
...
export async function PUT(req: NextRequest, { params }: { params: { username: string } }) {
  ...
  try {
    ...
    const { fullName, gender, designation, instituteName, companyName,
            ...
            avatarUrl, linkedinUrl, websiteUrl, profileCompleted } = body
    ...
    await db.query(
      `INSERT INTO user_profiles (user_id, full_name, gender, designation, institute_name, company_name,
        contact_number, email_id, total_exp, introduction, avatar_url, linkedin_url, website_url,
        profile_completed, post_count, follower_count, following_count, total_reads)
    ...
    return Response.json({
      profile: {
        fullName: p.full_name, designation: p.designation, instituteName: p.institute_name,
        companyName: p.company_name, introduction: p.introduction, avatarUrl: p.avatar_url,
