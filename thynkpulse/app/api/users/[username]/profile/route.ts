...
export async function PUT(req: NextRequest, { params }: { params: { username: string } }) {
  ...
  try {
    const {
      ...
      avatarUrl, linkedinUrl, websiteUrl,
      location, interests,
      profileCompleted
    } = await req.json()
    ...
    await db.query(
      `INSERT INTO user_profiles
         (user_id, full_name, gender, designation, institute_name, company_name,
          ...
          linkedin_url, website_url, location, interests, profile_completed,
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         location         = COALESCE(NULLIF($14,''), user_profiles.location),
         interests        = COALESCE(NULLIF($15,''), user_profiles.interests),
      ...
      [payload.userId, fullName||null, gender||null, designation||null,
       ...
       websiteUrl||null, location||null, interests||null, isComplete]
    )
    ...
    return Response.json({
      ...
      linkedinUrl: p.linkedin_url, location: p.location,
      interests: p.interests, profileCompleted: p.profile_completed,
    })
  } catch (err) {
