...
// Tracks user time-on-site sessions
// POST /api/analytics/session  — records a session
...
/* ── DB migration (run once) ────────────────────────────────────
   Add this to your migration file or run manually:
   ...
   CREATE INDEX IF NOT EXISTS idx_sessions_path    ON user_sessions(page_path);
─────────────────────────────────────────────────────────────── */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return Response.json({
      ...
      byDay:       byDay.rows.map(r => ({ day: r.day, sessions: Number(r.sessions), avgSec: Number(r.avg_sec) })),
      byPage:      byPage.rows.map(r => ({ path: r.page_path, visits: Number(r.visits), avgSec: Number(r.avg_sec) })),
      distribution: distribution.rows.map(r => ({ bucket: r.bucket, count: Number(r.count) })),
    })
  } catch (err) {
